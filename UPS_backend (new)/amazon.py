...
def AOrderHandler(dbconn, deal):
  try:
    ...
    #insert order
    if userid is None:
      query = "INSERT INTO PACKAGES (PACKAGE_ID, PACKAGE_STATUS, DESCRIPTION, COUNT, DST_X, DST_Y, WHID, SHIP_ID, USER_ID, TRUCK_ID) " 
    ...
    with mutex:
      ...
      while True:
        ...
        if truck is not None:
          truckid = truck[0]
          break
      query = "UPDATE TRUCKS SET TRUCK_STATUS = 'traveling' AND WHID = " + whid + " WHERE TRUCK_ID = " + truckid + ";"
      cur.execute(query)
    
    #add packages belonging to the selected truck
    for shipid in shipids:
      query = "UPDATE PACKAGES SET TRUCK_ID = " + truckid + " AND PACKAGE_STATUS = 'truck_en_route_to_warehouse' WHERE SHIP_ID = " + shipid + ";"
      cur.execute(query)
...
def AUpdateTStatusHandler(dbconn, updatestatus):
  try:
    ...
    #update truck status
    query = "UPDATE TRUCKS SET TRUCK_STATUS = '" + str(status) + "' WHERE TRUCK_ID = " + truckid + ";"
    cur.execute(query)
    ...
    for ship in ships:
      shipid = ship[0]
      if status == "loading":
        query = "UPDATE PACKAGES SET PACKAGE_STATUS = 'truck_loading' WHERE SHIP_ID = " + shipid + ";"
        cur.execute(query)
      else:
        query = "UPDATE PACKAGES SET PACKAGE_STATUS = 'truck_loaded' WHERE SHIP_ID = " + shipid + ";"
        cur.execute(query)
...
def ATruckGoDeliverHandler(dbconn, godeliver):
  try:
    ...
    query = "SELECT TRUCK_ID FROM TRUCKS WHERE TRUCK_ID = " + truckid + " AND (TRUCK_STATUS = 'loaded' OR TRUCK_STATUS = 'delivering');"
    cur.execute(query)
    truck = cur.fetchone()
    if truck is None:
    ...
    for shipid in shipids:
      query = "SELECT PACKAGE_STATUS FROM PACKAGES WHERE SHIP_ID = " + shipid + ";"
      cur.execute(query)
      pack = cur.fetchone()
    ...
    for shipid in shipids: 
      ...
      query = "SELECT DST_X, DST_Y FROM PACKAGES WHERE SHIPID = " + shipid + ";"
      cur.execute(query)
      p = cur.fetchone()
      package.x = p[0]
      package.y = p[1]
    ...
    for shipid in shipids:
      #update package status
      query = "UPDATE PACKAGES SET PACKAGE_STATUS = 'out_for_delivery' WHERE SHIP_ID = " + shipid + ";"
      cur.execute(query)
