...
def sayHelloToWorld(truck_num):
  ...
  try:
    ...
    for i in range(0, truck_num):
      ...
      truck.x = 0
      truck.y = 0
      query = "INSERT INTO TRUCKS (TRUCK_ID, TRUCK_STATUS, WHID, X, Y) VALUES (" + i + ", 'idle', -1, 0, 0);"
      cur.execute(query)
...
def UCompletionHandler(dbconn, completion):
  try:
    ...
    if status == "idle":
      query = "UPDATE TRUCKS SET TRUCK_STATUS = 'idle' AND X = " + x + " AND Y = " + y + " WHERE TRUCK_ID = " + truckid + ";"
      cur.execute(query)
      
    elif status == "arrive warehouse":
      query = "UPDATE TRUCKS SET TRUCK_STATUS = 'arrive_warehouse' AND X = " + x + " AND Y = " + y + " WHERE TRUCK_ID = " + truckid + ";"
      cur.execute(query)
      
      query = "SELECT WHID FROM TRUCKS WHERE TRUCK_ID = " + truckid + ";"
      cur.execute(query)
      whid = cur.fetchone()[0]
...
def UTruckStatusHandler(dbconn, truckstatus):
  try:
    ...
    #update truck status
    query = "UPDATE TRUCKS SET TRUCK_STATUS = '" + str(status) + "' AND X = " + x + " AND Y = " + y + " WHERE TRUCK_ID = " + truckid + ";"
    cur.execute(query)
    '''
